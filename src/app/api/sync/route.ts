...
type EpicOffer={title:string;keyImages:{type:string;url:string}[]};
...
async function highResolutionImages(offers:Giveaway[]){const images=new Map<number,string>();try{const response=await fetch("https://store-site-backend-static.ak.epicgames.com/freeGamesPromotions?locale=en-US&country=BR&allowCountries=BR",{cache:"no-store"});const json=await response.json();const epicOffers=(json?.data?.Catalog?.searchStore?.elements||[]) as EpicOffer[];for(const offer of offers.filter(item=>item.platforms.includes("Epic"))){const match=epicOffers.find(item=>gameTitle(offer.title).localeCompare(item.title,undefined,{sensitivity:"base"})===0);const image=match?.keyImages?.find(item=>item.type==="featuredMedia"||item.type==="OfferImageWide")?.url;if(image)images.set(offer.id,image)}}catch{}await Promise.all(offers.filter(item=>item.platforms.includes("Steam")).map(async offer=>{try{const response=await fetch(`https://store.steampowered.com/api/storesearch/?term=${encodeURIComponent(gameTitle(offer.title))}&l=english&cc=br`,{cache:"no-store"});const json=await response.json();const match=json?.items?.find((item:{name:string})=>gameTitle(offer.title).localeCompare(item.name,undefined,{sensitivity:"base"})===0)||json?.items?.[0];if(match?.id)images.set(offer.id,`https://cdn.cloudflare.steamstatic.com/steam/apps/${match.id}/library_hero.jpg`)}catch{}}));return images}
async function synchronize(request:NextRequest){
 ...
 const heroImages=await highResolutionImages(eligible);
