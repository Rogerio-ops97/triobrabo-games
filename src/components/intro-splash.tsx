...
export function IntroSplash({images}:{images:string[]}){
 const [visible,setVisible]=useState(true);const [exiting,setExiting]=useState(false);const [displayImages,setDisplayImages]=useState<string[]>([]);
 useEffect(()=>{setTimeout(()=>setDisplayImages(freshSelection(images)),0);const reduced=window.matchMedia("(prefers-reduced-motion: reduce)").matches;const exitTimer=window.setTimeout(()=>setExiting(true),reduced?100:5250);const closeTimer=window.setTimeout(()=>setVisible(false),reduced?250:6000);return()=>{clearTimeout(exitTimer);clearTimeout(closeTimer)}},[images]);
