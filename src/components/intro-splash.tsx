...
export function IntroSplash({images}:{images:string[]}){
 ...
 useEffect(()=>{if(localStorage.getItem("triobrabo:intro-enabled")==="0"||sessionStorage.getItem("triobrabo:intro-shown")==="1")return;sessionStorage.setItem("triobrabo:intro-shown","1");queueMicrotask(()=>{setDisplayImages(freshSelection(images));setVisible(true)});const reduced=window.matchMedia("(prefers-reduced-motion: reduce)").matches;const exitTimer=window.setTimeout(()=>setExiting(true),reduced?100:5250);const closeTimer=window.setTimeout(()=>setVisible(false),reduced?250:6000);const preference=(event:Event)=>{if(!(event as CustomEvent<boolean>).detail){setExiting(true);window.setTimeout(()=>setVisible(false),350)}};window.addEventListener("triobrabo:intro-preference",preference);return()=>{clearTimeout(exitTimer);clearTimeout(closeTimer);window.removeEventListener("triobrabo:intro-preference",preference)}},[images]);
